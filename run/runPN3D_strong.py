...
case_consts = [' --baseflow=1 --force=1  --domain=1 --nf=8  --tolNOX=1.e-6  --tolInnerBelos=1.e-1 --maxIter=2 --lx=8. --ly=2. --lz=4.']
...
for n in nxs:
	...
	for np in nps:
		...
		os.system(' rm ./* -r -v  ')
		case_para = ' --nx='+str(96*n+1)+' --ny='+str(24*n+1)+' --nz='+str(48*n+1)+' --npx='+str(np)+' --npy='+str( max(np/4,1) )+' --npz='+str(max(np/2,1))+' --maxGrids='+str(int(2+n))+' --re='+str(re)+' --alpha2='+str(alpha2)+' ' 
		...
		for run in range(runs):
			os.system( exe_pre(np*max(np/2,1)*max(np/4,1),' -R "select[model==Opteron6174"] -W 8:00',run)+exe_path+exe+case_para+case_consts[0] +'  ' )
