...
case_path[0] = '/scale_time_re_'+str(re)+'_alpha2_'+str(alpha2)+'nx_'+str(nx)+'npx_'+str(npx)
mkdir( case_path, 0 )
	...
	for npf in range( 1, nf+2 ):
		...
		if( True ):
			...
			for run in runs:
				print()
				print( case_path )
				print(     exe_pre( nptot, ' -N -R "select[model==Opteron8380"] -R "rusage[mem='+str(1024*1)+']" ', run ) + exe_path+'/'+exe  )
				os.system( exe_pre( nptot, ' -N -R "select[model==Opteron8380"] -R "rusage[mem='+str(1024*1)+']" ', run ) + exe_path+'/'+exe  )
