...
contains
    ...
    !> \brief computes \f$ \mathrm{TimeNS = \nabla\phi } \f$
    ...
    subroutine OP_TimeNS( &
        ...
        real(c_double), intent(out) :: TimeNS(bL(1):(N(1)+bU(1)),bL(2):(N(2)+bU(2)),bL(3):(N(3)+bU(3)))
        ...
        if( 1==dir ) then
            ...
            do k = SS(3), NN(3)
                do j = SS(2), NN(2)
                    do i = SS(1), NN(1)
                        TimeNS(i,j,k) = c(gL(1),i)*phi(i+gL(1),j,k)
                        !pgi$ unroll = n:8
                        do ii = gL(1)+1, gU(1)
                            TimeNS(i,j,k) = TimeNS(i,j,k) + c(ii,i)*phi(i+ii,j,k)
                        end do
                    end do
                end do
            end do
        ...
        if( 2==dir ) then
            ...
            do k = SS(3), NN(3)
                do j = SS(2), NN(2)
                    do i = SS(1), NN(1)
                        TimeNS(i,j,k) = c(gL(2),j)*phi(i,j+gL(2),k)
                        !pgi$ unroll = n:8
                        do jj = gL(2)+1, gU(2)
                            TimeNS(i,j,k) = TimeNS(i,j,k) + c(jj,j)*phi(i,j+jj,k)
                        end do
                    end do
                end do
            end do
        ...
        if( 3==dir )then
            ...
            do k = SS(3), NN(3)
                do j = SS(2), NN(2)
                    do i = SS(1), NN(1)
                        TimeNS(i,j,k) = c(gL(3),k)*phi(i,j,k+gL(3))
                        !pgi$ unroll = n:8
                        do kk = gL(3)+1, gU(3)
                            TimeNS(i,j,k) = TimeNS(i,j,k) + c(kk,k)*phi(i,j,k+kk)
                        end do
                    end do
                end do
            end do
