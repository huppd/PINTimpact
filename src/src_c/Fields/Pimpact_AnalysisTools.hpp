...
typename MultiHarmonicFieldT::GridT::Scalar
truncErrorEstimate(const MultiHarmonicFieldT& field, ENorm normType=ENorm::L2) {

  using ST = typename MultiHarmonicFieldT::GridT::Scalar;
  using OT = typename MultiHarmonicFieldT::GridT::Ordinal;
  ...
  if(0<nf and grid->si(F::U, 3)<=1 and 1<=grid->ei(F::U, 3))
    u_1  = field.getField(1).norm(normType);

  if(0<nf and grid->si(F::U, 3)<=nf and grid->nGlo(3)<=nf)
    u_nf = field.getField(nf).norm(normType);

  int rank_1 = 0;
  if(1 == grid->getProcGrid()->getNP(3))
    rank_1 = 0;
  else if(0 == (grid->nGlo(3)+1)%grid->getProcGrid()->getNP(3))
    rank_1 = 1;
  int rank_nf = grid->getProcGrid()->getNP(3)-1;
  ...
  MPI_Ibcast(
      ...
      grid->getProcGrid()->getCommBar(3), // communicator (handle)
  ...
  MPI_Ibcast(
      ...
      grid->getProcGrid()->getCommBar(3), // ccommunicator (handle) ommunicator (handle)
...
void writeMHLambda2(MHFieldT& x, int id) {
  ...
  using GridT = typename MHFieldT::GridT;
...
void writeLambda2(VectorFieldT& x, int id) {
  ...
  using GridT = typename VectorFieldT::GridT;

  using ST = typename GridT::Scalar;
  using OT = typename GridT::Ordinal;
  using SFT = ScalarField<GridT>;
  ...
  //==============================================================================
  {
    auto interpolateV2S = grid->getInterpolateV2S();
    auto interpolateS2V = create<InterpolateS2V>(grid);
  ...
  x.exchange();
  ScalarField<GridT> res(grid);

  auto divOp = create<DivOp>(grid);
  ...
  for(OT k=grid->si(F::S, Z); k<=grid->ei(F::S, Z); ++k)
    for(OT j=grid->si(F::S, Y); j<=grid->ei(F::S, Y); ++j)
      for(OT i=grid->si(F::S, X); i<=grid->ei(F::S, X); ++i) {
        ...
        //--- d/dx ----
        for(int ii=grid->dl(X); ii<=grid->du(X); ++ii) {
        ...
        //--- d/dy ------------------------------------------
        for(int jj=grid->dl(Y); jj<=grid->du(Y); ++jj) {
        ...
        //--- d/dz ------------------------------------------
        for(int kk=grid->dl(Z); kk<=grid->du(Z); ++kk) {
...
void writeSpectrum(const MultiHarmonicFieldT& field,
  ...
  if(field.global() == MultiHarmonicFieldT::Global::Y)
    ...
    Teuchos::RCP<MultiHarmonicFieldT> temp =
      Teuchos::rcp(new MultiHarmonicFieldT(grid, MultiHarmonicFieldT::Global::Y));
    *temp = field;
    y = temp;
  }
  y->exchange();
...
void computeEnergyDir(const VectorField<GridT>& vel, std::ostream& out=std::cout,
    const ECoord dir=ECoord::Y, const typename GridT::Scalar gamma=0.) {
  ...
  ScalarField<GridT> temp(grid);

  std::vector<ST> energy(grid->nLoc(dir), Teuchos::ScalarTraits<ST>::zero());

  for(F f=F::U; f<GridT::sdim; ++f) {

    grid->getInterpolateV2S()->apply(vel(f), temp);
  ...
  if(0 == grid->getProcGrid()->getRankSlice(dir)) {
    MPI_Gather(
        energy.data(),                           // void* send_data,
        grid->nLoc(dir)-1,                      // int send_count,
        MPI_REAL8,                               // MPI_Datatype send_datatype,
        energyGlobal.data(),                     // void* recv_data,
        grid->nLoc(dir)-1,                      // int recv_count,
        MPI_REAL8,                               // MPI_Datatype recv_datatype,
        0,                                       // int root,
        grid->getProcGrid()->getCommBar(dir)); // MPI_Comm communicator);
...
void computeHEEnergyDir(
    const VectorField<GridT>& vel,
    std::ostream& out=std::cout,
    const typename GridT::Scalar gamma=10.,
  ...
  OT N1p = grid->ei(F::S, X);
  OT S2p = grid->si(F::S, Y);
  OT N2p = grid->ei(F::S, Y);
  OT S3p = grid->si(F::S, Z);
  OT N3p = grid->ei(F::S, Z);
  ...
  ScalarField<GridT> vel_dist0(grid);
  ScalarField<GridT> vel_dist1(grid);
  ScalarField<GridT> vel_dist2(grid);

  ScalarField<GridT> vel_mode_0(grid);
  ScalarField<GridT> vel_mode_1(grid);
  ScalarField<GridT> vel_mode_2(grid);

  grid->getInterpolateV2S()->apply(vel(F::U), vel_dist0);
  grid->getInterpolateV2S()->apply(vel(F::V), vel_dist1);
  grid->getInterpolateV2S()->apply(vel(F::W), vel_dist2);


  Teuchos::SerialDenseMatrix<OT, ST> He(3, n_Hermitemodes);
  Teuchos::SerialDenseMatrix<OT, ST> He_global(3, n_Hermitemodes);
  Teuchos::SerialDenseMatrix<OT, ST> energydensity(n_Hermitemodes, grid->nLoc(dir));
  ...
  for(OT j=S2p; j<=N2p; ++j) {        // spanwise
    for(OT i=S1p; i<=N1p; ++i) {       // wall-normal
      ...
      MPI_Allreduce(He.values(), He_global.values(), 3*n_Hermitemodes, MPI_REAL8, MPI_SUM, grid->getProcGrid()->getCommBar(Z));

      // --- integral over x-dimension: \int_0^\infty u_kn \dx
      //ST dx1p = coord->dx(F::S, X, i);
      //for(OT n = 0; n<n_Hermitemodes; ++n) {
      //energydensity(n, j-grid->si(F::S, Y)) +=
      ...
      ST dx1p = coord->dx(F::S, X, i);
      energydensity(0, j-grid->si(F::S, Y)) +=
  ...
  Teuchos::SerialDenseMatrix<OT, ST> energyGlobal(n_Hermitemodes, grid->nGlo(dir));

  if(0 == grid->getProcGrid()->getRankSlice(dir)) {
    MPI_Gather(
        energydensity.values(),                  // void* send_data,
        (grid->nLoc(dir)-1)*n_Hermitemodes,     // int send_count,
        MPI_REAL8,                               // MPI_Datatype send_datatype,
        energyGlobal.values(),                     // void* recv_data,
        (grid->nLoc(dir)-1)*n_Hermitemodes,     // int recv_count,
        MPI_REAL8,                               // MPI_Datatype recv_datatype,
        0,                                       // int root,
        grid->getProcGrid()->getCommBar(dir)); // MPI_Comm communicator);
