...
using SpaceT = Pimpact::Space<S,O,4,2>;
	...
	for( int refine=0; refine<refinement; ++refine ) {
		...
		if( withprec>0 ) {
			...
			if( withprec>1 ) {
				...
				//Teuchos::RCP< Pimpact::ModeNonlinearOp< Pimpact::ConvDiffOpT<SpaceT> > >
					auto modeOp =
			...
			{ // init multigrid divgrad
				divGradInv2->setRightPrec( Pimpact::createMultiOperatorBase( Pimpact::create< Pimpact::DivGradO2Inv>(Pimpact::create<Pimpact::DivGradO2Op>(space)) ) );
				//divGradInv2->setLeftPrec( Pimpact::createMultiOperatorBase( Pimpact::create< Pimpact::DivGradO2Inv>(Pimpact::create<Pimpact::DivGradO2Op>(space)) ) );
			}
