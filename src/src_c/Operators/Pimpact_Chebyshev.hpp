...
//#endif // HAVE_TEUCHOS_DEBUG
	...
	void applyBarrett93( const DomainFieldT& b, RangeFieldT& x0 ) const {
		...
		Teuchos::RCP<DomainFieldT> x = x0.clone( DeepCopy );

		// r_0 = B-Ax_0
		Teuchos::RCP<RangeFieldT> r = x0.clone( ShallowCopy );
		Teuchos::RCP<RangeFieldT> z = x0.clone( ShallowCopy );
		Teuchos::RCP<RangeFieldT> p = x0.clone( ShallowCopy );
	...
	void applyPIMP( const DomainFieldT& b, RangeFieldT& x0 ) const {
		...
		//
		Teuchos::RCP<RangeFieldT> r = b.clone( ShallowCopy );

		Teuchos::RCP<DomainFieldT> p = x0.clone( ShallowCopy ); // ~ init(0)
		Teuchos::RCP<DomainFieldT> x = x0.clone( DeepCopy ); // x->assign( x0 )
...
public:
	...
	const std::string getLabel() const { return( "Chebyshev" ); };
