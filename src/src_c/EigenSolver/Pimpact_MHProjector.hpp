...
class DGProjector {
  ...
  ScalarField<SpaceT> nullspaceV_;
  ScalarField<SpaceT> nullspaceS_;

public:

  DGProjector() {}
  DGProjector( const Teuchos::RCP<const OperatorT>& op):
    nullspaceV_( op->space() ),
    nullspaceS_( op->space() ) {

    DivGradNullSpace<DivOp<SpaceT> > compNullspace;

    compNullspace.computeNullSpace( op->getOPS()/*->get*/->getDivOp(), nullspaceS_ );
  }
