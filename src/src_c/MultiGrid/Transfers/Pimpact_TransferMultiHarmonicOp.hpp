...
//
		...
		Ordinal iS = std::max( y.space()->sInd(U,3), 0 );
		Ordinal iE = std::min( x.space()->nGlo(3), y.space()->eInd(U,3) );
      ...
      op_->apply( x.getConstSField(i), y.getSField(i) );
		...
		iS = std::max(x.space()->nGlo(3),y.space()->sInd(U,3));
		iE = y.space()->eInd(U,3);
		for( Ordinal i=iS; i<iE; ++i ) {
