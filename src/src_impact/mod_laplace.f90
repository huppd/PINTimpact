...
contains
  ...
  subroutine relaxation_div_grad(init_yes,n_relax,g,bb,rel) ! TEST!!! reine 2D-Variante fehlt noch ... ! TEST!!! aufraeumen und Variablen substituieren ...
    ...
    !    - line relaxation runs on Rosa (Cray XT5, Istanbul hex-core processors) in single coremode with      !
    ...
    ! Allgemein, pgf90: "Invariant if transformation" <==> IF-Statements werden
    ...
    !              - Bei der Initialisierung müssen die Intervallgrenzen SiR:NiR anstelle von SiiR:N!
    !                gewählt werden, da beim  Aufbau der RHS ("vec") innerhalb der Linienrelaxation !
    !                die Randbereiche SiR und NiR aufgebaut aber ggf. mit einer Korrektur wieder übe!
    !                werden. Ansonsten wäre eine Initialisierung allein im Feldbereich SiiR:NiiR aus!
    !                kann aber z.B. zu Floating Point Exceptions führen (die für die eigentliche Rec!
    !                allerdings irrelevant wären)!                                                  !
    !              - obere Stirnflächen werden bei der Initialisierung ebenfalls berücksichtigt, da !
    !                verschiedene Richtungen nacheinander bearbeitet werden und dies beim Aufbauen d!
    !                Seite sonst berücksichtigt werden müsste. ==> geringerer Mehraufwand beim Rechn!
    !                weniger Programmierdetails.                                                    !
    !              - LU-Zerlegung (d.h. band, mult) kann gespeichert werden, wenn mindestens eine Ri!
    !                äquidistant ist. Der Lösungsaufwand würde sich etwa halbieren!!                !
    !              - "r == 1 .AND. init_yes" sollte idealerweise aus den Schleifen herausgezogen wer!
        ...
        if (r == 1 .and. init_yes) then
          ...
          if (BC_1L > 0) then
            ...
            do k = S33R, N33R
              !pgi$ unroll = n:8
              do j = S22R, N22R
                comp(i,j,k) = omega*(bb(i,j,k) - cdg1( 1,i,g)*rel(i+1,j,k)) / cdg1(0,i,g) + (1.-)
              end do
            end do
          end if
          !-------------------------------------------------------------------------------------
      ...
      do r = 1, n_relax
        ...
        if (impl_dir(1) == 0 .and. impl_dir(2) == 0 .and. impl_dir(3) == 0) then
          ...
          if (RBGS_mode > 0) then
            !-------------------------------------------------------------------------------------------------!
            ! Hinweis:                                                                                        !
            ! Die Red-Black-Sortierung sollte für eine gute Performance global sein, d.h. ueber die Blócke    !
      ...
      do r = 1, n_relax
        ...
        if (impl_dir(1) == 0 .and. impl_dir(2) == 0 .and. impl_dir(3) == 0) then
          ...
          if (RBGS_mode > 0) then
            !-------------------------------------------------------------------------------------------------!
            ! Hinweis:                                                                                        !
            ! Die Red-Black-Sortierung sollte für eine gute Performance global sein, d.h. ueber die Blücke    !
            ! hinweg implementiert sein. Die aktuelle Umsetzung macht sich zunutze, dass im Parallelbetrieb   !
            ! die Anzahl der Gitterpunkte in jeder Raumrichtung automatisch eine gerade Zahl ist (die Tat-    !
            ! sache, dass in Randblöcken ggf. eine ungerade Anzahl Gitterpunkte abgearbeitet wird hat damit   !
            ! nichts zu tun!). Daher kann sich der Shift "ss" in jedem Block einfach auf den lokalen Gitter-  !
