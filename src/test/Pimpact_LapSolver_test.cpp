...
TEUCHOS_UNIT_TEST( bla, bla  ) {
	...
	auto rhs = x->clone( Pimpact::ShallowCopy );
	auto rhs2 = x->clone( Pimpact::ShallowCopy );
	...
	//{ // compute contribution to nullspace
		//auto fSlevel = rhs->getSFieldPtr()->clone( Pimpact::DeepCopy );
	...
	//xl->init(0.);
	////rhs->getSField().level();
	//auto f_ = rhs->getSFieldPtr()->clone( Pimpact::DeepCopy );
	//f_->get0FieldPtr()->add(
	//1., rhs->getSFieldPtr()->get0Field(),
